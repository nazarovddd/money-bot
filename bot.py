...
# Стабильная инициализация финальной базы данных версии v7
...
def ask_income(message):
    # Принудительно очищаем прошлые зависшие шаги ожидания перед запуском нового
    bot.clear_step_handler_by_chat_id(chat_id=message.chat.id)
...
def process_income(message):
    try:
        # Проверяем, не нажал ли пользователь другую команду меню вместо ввода цифр
        ...
        msg = bot.send_message(message.chat.id, "⚠️ Вводите только цифры слитно без пробелов:")
        bot.register_next_step_handler(msg, process_income)
...
def view_analytics(message):
    ...
    spent_today = row_t if row_t and row_t is not None else 0.0
...
def process_expense_amount(message):
    try:
        ...
        msg = bot.send_message(message.chat.id, "⚠️ Вводите сумму только цифрами:")
        bot.register_next_step_handler(msg, process_expense_amount)
...
def process_expense_category(call):
    ...
    today_spent = row_today if row_today and row_today is not None else 0.0
...
def process_debt_give(message):
    try:
        ...
        msg = bot.send_message(message.chat.id, "⚠️ Ошибка ввода! Пишите строго по образцу: `Имя 50000`:")
        bot.register_next_step_handler(msg, process_debt_give)
...
def process_debt_return(message):
    try:
        ...
        msg = bot.send_message(message.chat.id, "⚠️ Ошибка! Пишите строго по образцу: `Имя 20000`:")
        bot.register_next_step_handler(msg, process_debt_return)
